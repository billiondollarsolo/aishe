...
| Usage ledger | `<data>/usage.jsonl` | Always written; numbers only, no prompts, answers, commands, or paths; rotates at 8 MiB with one `.1` generation | `aishe usage --json` | `aishe uninstall --audit-undo --dry-run` |

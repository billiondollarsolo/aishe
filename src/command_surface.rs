...
/// What running the command changes, in the words the help, palette, and docs
/// all use. Declared per command rather than derived from two orthogonal enums,
/// which produced nine labels and several wrong ones.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Effect {
    /// Prints and exits.
    ReadOnly,
    /// Changes this shell session only.
    ThisShell,
    /// Changes this shell; `--default` (or the follow-up prompt) also saves it.
    ThisShellOrDefault,
    /// Writes config.toml.
    SavesConfig,
...
pub struct CommandSpec {
    ...
    /// The one label every user-facing surface prints.
    pub effect: Effect,
    /// Still dispatched and tab-completed, but kept out of help rows, the
    /// palette, and the docs table: duplicates and maintenance affordances.
    pub hidden: bool,
...
pub static COMMANDS: &[CommandSpec] = &[
    ...
    CommandSpec {
        ...
        hidden: false,
        effect: Effect::ThisShellOrDefault,
...
mod tests {
    ...
    fn registry_is_valid_and_complete_for_compatibility_line() {
        ...
        let active: Vec<_> = COMMANDS
            ...
            .map(|spec| spec.id)
            .collect();
        // The 0.6.5 tombstones were dropped in 0.7.0; the Lifecycle machinery
        // stays for the next removal.
        assert_eq!(active, ACTIVE_IDS);
        assert!(COMMANDS.iter().all(|spec| spec.is_active()));
    }
